...
>(({ className, ...props }, ref) => (
  <thead ref={ref} className={cn('[&_tr]:border-b border-slate-200/80 bg-slate-50/80 sticky top-0 z-10', className)} {...props} />
));
...
>(({ className, ...props }, ref) => (
  <th
    ref={ref}
    className={cn(
      'h-10 px-4 text-left align-middle text-[11px] font-bold text-slate-500 uppercase tracking-wider [&:has([role=checkbox])]:pr-0',
      className
    )}
    {...props}
  />
...
>(({ className, ...props }, ref) => (
  <td
    ref={ref}
    className={cn('p-4 align-middle [&:has([role=checkbox])]:pr-0 text-xs', className)}
    {...props}
  />
));
