...
export const AstRuleBuilder: React.FC<AstRuleBuilderProps> = ({
  initialAst,
  onChange,
  readOnly = false,
}) => {
  ...
  return (
    <div className="space-y-4 text-slate-900 w-full max-w-full overflow-hidden">
      ...
      {mode === 'BUILDER' ? (
        /* Visual Config Panel with Internal Smooth Scrolling */
        <div className="space-y-3 w-full max-w-full">
          ...
          <div className="space-y-4 w-full max-w-full overflow-x-auto pb-2">
            {groups.map((group, gIdx) => (
              <div key={group.id} className="space-y-2 min-w-[580px] max-w-full">
                ...
                {/* Group Box with Unified Fixed Left Gutter (pl-9) for 100% Horizontal Alignment */}
                <div className="relative pl-9 space-y-3">
                  ...
                  {group.conditions.map((condition, cIdx) => (
                    <div key={condition.id} className="relative">
                      ...
                      {group.conditions.length > 1 && (
                        <div className="absolute -left-6 top-1/2 -translate-y-1/2 w-6 border-t-2 border-dashed border-indigo-300 pointer-events-none" />
                      )}
        ...
        <div className="space-y-2 w-full max-w-full">
          <div className="flex items-center justify-between text-xs text-slate-500 font-semibold">
            ...
            <span className="text-[11px] text-slate-400">{readOnly ? 'Read-only specification' : 'Two-way synchronized'}</span>
          </div>
          <textarea
            rows={10}
            value={rawJson}
            ...
            className={`w-full font-mono text-[11px] p-3.5 rounded-xl border border-slate-200 bg-slate-900 text-emerald-400 focus:outline-none focus:ring-2 focus:ring-slate-950 leading-relaxed max-w-full overflow-x-auto ${
      ...
      {/* Live Generated Expression Preview with Internal Scroll */}
      <div className="p-3 rounded-xl bg-slate-950 text-slate-100 font-mono text-[11px] space-y-1 shadow-xs border border-slate-800 max-w-full overflow-hidden">
        ...
        <div className="max-h-24 overflow-y-auto overflow-x-auto pr-1">
          <p className="text-emerald-300 font-semibold break-all whitespace-pre-wrap">
...
const ConditionRow: React.FC<{
  ...
  onAddOr: () => void;
  onDelete: () => void;
}> = ({ condition, canDelete, readOnly = false, onUpdate, onAddAnd, onAddOr, onDelete }) => {
  const isMultiValue =
    ...
    <div className="space-y-1">
      <div className="p-2.5 rounded-xl bg-white border border-slate-200 shadow-2xs flex flex-col md:flex-row md:items-center gap-2">
        ...
        <div className="flex-1 min-w-[160px] max-w-full">
          {isMultiValue ? (
            <div className="min-h-[36px] max-h-24 overflow-y-auto p-1.5 rounded-lg border border-slate-200 bg-white flex flex-wrap items-center gap-1.5 focus-within:ring-1 focus-within:ring-slate-950">
              {tags.map((tag, idx) => (
                <span
                  key={idx}
        ...
        {/* 4. Action Buttons (And, Or, Trash) - Hidden in readOnly mode */}
        {!readOnly && (
